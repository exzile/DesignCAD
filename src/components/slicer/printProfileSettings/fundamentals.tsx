  ...
  return (
    <SlicerSection title="Walls" color="#a78bfa" defaultOpen={false}>
      ...
      <Tier min="advanced">
        ...
        <Sel label="Wall Generator" value={print.wallGenerator ?? 'arachne'} onChange={(v) => upd({ wallGenerator: v as 'classic' | 'arachne' })} options={[{ value: 'classic', label: 'Classic (fixed-width offset)' }, { value: 'arachne', label: 'Arachne (variable-width)' }]} helpBrief="Variable-width walls handle narrow regions cleanly. Classic is the legacy fixed-width offset." onShowHelp={() => showHelp('wallGenerator', 'Wall Generator')} />
      </Tier>
      <Tier min="expert">
